...
```bash
python checkout.py <repo> --path-only
```

Examples:

```bash
python checkout.py mitsuhiko/minijinja --path-only
python checkout.py github.com/mitsuhiko/minijinja --path-only
python checkout.py https://github.com/mitsuhiko/minijinja --path-only
```
...
```bash
python checkout.py <repo> --force-update --path-only
```

## Recommended workflow

1. Resolve repository path via `checkout.py --path-only`.
