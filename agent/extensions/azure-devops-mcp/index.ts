...
export default function (pi: ExtensionAPI) {
  const registerAdoTool = async (ctx: ExtensionContext) => {
  ...
  // // See Dynamic Tools Extension in pi repo, this way you don't slow down pi's startup
  // pi.on("session_start", async (_event, ctx) => {
  //   registerAdoTool(ctx);
  // });

  pi.registerCommand("add-ado-tools", {
    description: "Azure devops tool, read tickets, messages in ADO",
    handler: async (args, ctx) => {
      const created = registerAdoTool(ctx);
      const toolName = ''
      if (!created) {
